...
export const changelog: ChangelogEntry[] = [
  {
    ...
    date: '2026-09-07',
    added: [
      '📺 追更提醒推送显示资源站：推送消息中每个剧集名称后以括号附带资源站名称（如“海贼王（如意资源）”），无来源信息时保持原格式',
    ],
    changed: [
      '📦 依赖升级（同步上游 45bd6852..4253f09a）：hls.js 升级至 1.7.2、@tanstack/react-query 升级至 5.102.6、@tanstack/react-virtual 升级至 3.14.10、@mui/material 升级至 9.4.0',
      '📄 文档更新（同步上游）：更新 Cloudflare Worker 代理加速配置指南，补充 Bangumi/TMDB 代理、播放流三级 fallback、Emby 源跳过等说明',
    ],
    fixed: [
      // 无修复内容
    ],
  },
  {
    ...
    date: '2026-06-14',
    added: [
      '🎛️ 后台顶部菜单显示控制：分类配置页可控制源浏览器、电影、剧集、短剧、动漫、综艺等固定导航入口显隐，保存后刷新 layout 运行时配置',
      '⚙️ 长按倍速设置增强：后台新增站点级默认长按倍速，前台设置支持本地选择、恢复后台默认，并支持右箭头长按倍速',
      '🕐 播放器右上角时间显示模式：新增关闭、常显、随控制栏三种显示模式，播放中可实时生效',
      '🎮 播放器全屏跳集与截图控制：全屏新增上一集按钮，移动端播放器新增截图按钮，并使用紧凑尺寸避免遮挡其他按钮',
    ...
    changed: [
      '🎬 播放器控制栏适配：移动端隐藏弹幕按钮、关闭网页全屏，仅保留系统全屏；上一集仅在全屏显示，右上角时间默认随控制栏，减少按钮挤占和字幕干扰',
      '🎚️ 播放器清晰度徽章：左下角清晰度显示改为跟随控制栏同步淡入淡出，控制栏隐藏时不再单独停留',
      '🔗 版本信息入口：将“前往仓库”按钮统一指向当前项目仓库地址',
      '🧭 播放页导航与竖屏布局：恢复播放页导航 Shell，减少竖屏布局下的音频控件残留',
      '📺 选集与测速入口：重构选集布局和测速按钮位置，优化搜索卡片源 hover 与收藏菜单交互',
      '📦 依赖升级：@tanstack/react-query 升级至 5.101.0',
      '📦 依赖升级：@mui/material 升级至 9.1.2，@tanstack/react-virtual 升级至 3.14.4',
    ],
    fixed: [
      '⏭️ 自动下一集：恢复播放结束自动跳下一集，增加 1 秒延迟和同集确认，避免 Wake Lock 监听遗漏或误跳',
      '⚡ 长按倍速：修复恢复默认、后台默认值继承、实际播放速率不一致等问题',
      '📱 长按倍速手势：移动端长按倍速触发后，手指移动不再退出倍速，也不会误拖动进度条，松手后再恢复原速',
      '📅 Bangumi 日历：服务端日历获取时校验数组响应，避免异常数据导致崩溃',
      '🎚️ 播放器与测速 UI：修复播放速率提示文案格式、测速按钮边距回退和搜索卡片交互细节',
      ...
      '🌐 Bangumi CN 区修复：修复中国区 API 被拦截导致首页崩溃问题，新增多层智能降级策略；修复 useState 初始值、localStorage 重置及管理面板缺少默认 cmliussss 的问题',
