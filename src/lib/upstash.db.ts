...
export class UpstashRedisStorage implements IStorage {
  ...
  async deleteUser(userName: string): Promise<void> {
    ...
    await withRetry(() => this.client.del(this.reminderHashKey(userName)));
    await withRetry(() =>
      this.client.del(this.watchingFollowHashKey(userName)),
    );
    await withRetry(() => this.client.del(this.skipHashKey(userName)));
    await withRetry(() => this.client.del(this.skipMetaHashKey(userName)));
  ...
  private async touchSkipConfigMeta(userName: string): Promise<void> {
    ...
    await withRetry(() => this.client.hincrby(metaKey, 'revision', 1));
    await withRetry(() =>
  ...
  async deleteSkipConfig(
    ...
    const hashKey = this.skipHashKey(userName);
    const field = this.skipField(source, id);
    const removed = await withRetry(() => this.client.hdel(hashKey, field));
    const legacyField = legacySkipConfigKey(source, id);
    if (legacyField && legacyField !== field) {
      const legacyRemoved = await withRetry(() =>
  ...
  async getSkipConfigsMeta(userName: string): Promise<SkipConfigMeta | null> {
    const metaKey = this.skipMetaHashKey(userName);
    const raw = await withRetry(() => this.client.hgetall(metaKey));
    ...
    const count = await withRetry(() =>
      this.client.hlen(this.skipHashKey(userName)),
    );
