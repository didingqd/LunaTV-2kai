...
const VideoCard = forwardRef<VideoCardHandle, VideoCardProps>(
  function VideoCard(
    ...
    useEffect(() => {
      if (!isBangumi || !favoriteSource || !favoriteId || !releaseDate) return;
      saveBangumiDateRemarkIfAllowed(
        favoriteSource,
        favoriteId,
        releaseDate,
    ...
    const handlePushRemark = useCallback(async () => {
      if (!favoriteSource || !favoriteId || !customRemark.trim()) return;
      if (
        !window.confirm(
          '将当前备注推送给所有用户，已有手动备注的用户不会被覆盖。继续吗？',
        )
      ) {
        return;
      }
      ...
      try {
        const result = await pushVideoRemarkToAll(favoriteSource, favoriteId);
        window.alert(
          `推送完成：更新 ${result.updatedUsers} 个用户，写入 ${result.insertedRecords} 条备注。`,
        );
      } catch (error) {
        window.alert(error instanceof Error ? error.message : '推送失败');
      }
    }, [favoriteSource, favoriteId, customRemark]);
    ...
    const canShowFavoriteAction =
      config.showHeart && !!favoriteSource && !!favoriteId;
      ...
      <>
        ...
        >
          ...
          >
            ...
            {/* 操作按钮 - hover显示（非收藏页面） */}
            {(canShowFavoriteAction || config.showCheckCircle) &&
              from !== 'favorite' && (
                <div
                  data-button='true'
            ...
            {/* 修改点：备注徽章固定在右侧、上移一行（bottom-9），与右下角的删除/收藏按钮不在同一行，备注较长时也不会遮挡按钮 */}
            {customRemark && (
              <div
                className='absolute bottom-9 right-2 max-w-[70%] truncate bg-black/70 backdrop-blur-sm px-2 py-0.5 rounded-md shadow-lg text-white/85 text-[10px] font-medium transition-all duration-300 ease-out group-hover:scale-105 z-30'
                style={
                  {
                    WebkitUserSelect: 'none',
                    userSelect: 'none',
                        ...
                        return (
                          ...
                          >
                            ...
                            >
                              ...
                              {/* 小箭头 */}
                              <div
                                className={`absolute top-full ${from === 'search' ? 'left-2 sm:left-3' : 'right-2 sm:right-3'} w-0 h-0 border-l-[4px] border-r-[4px] border-t-[4px] sm:border-l-[6px] sm:border-r-[6px] sm:border-t-[6px] border-transparent border-t-gray-800/90`}
                              ></div>
                            </div>
                          </div>
                        );
