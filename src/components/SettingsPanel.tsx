...
export const SettingsPanel = memo(({ isOpen, onClose }: SettingsPanelProps) => {
  ...
  >('standard');
  const [danmuSegmentLimit, setDanmuSegmentLimit] = useState(
    DEFAULT_DANMU_SEGMENT_LIMIT,
  );
  const [danmuSegmentLimitInput, setDanmuSegmentLimitInput] = useState(
    String(DEFAULT_DANMU_SEGMENT_LIMIT),
  );
  ...
  const [preferLocationAssignNavigation, setPreferLocationAssignNavigation] =
    useState(false);
  const [lockedLongPressRate, setLockedLongPressRate] = useState(
    DEFAULT_LOCKED_LONG_PRESS_RATE,
  );
  ...
  useEffect(() => {
    ...
    setPreferLocationAssignNavigation(
      readLS(
        ...
        RC.PREFER_BROWSER_NAVIGATION === true,
      ),
    );
    const fmt = localStorage.getItem('downloadFormat');
    if (fmt === 'TS' || fmt === 'MP4') setDownloadFormat(fmt);
    // 修改点：长按倍速默认值改为优先读取本地设置，无本地设置时继承后台站点配置
    const runtimeLockedLongPressRate = sanitizeLongPressRate(
      Number(RC.DEFAULT_LOCKED_LONG_PRESS_RATE),
    );
    const storedLockedLongPressRate = localStorage.getItem(
      LOCKED_LONG_PRESS_RATE_KEY,
    );
    ...
    const loadedDanmuLimit = loadDanmuSegmentLimit(getSettingsUsername());
    setDanmuSegmentLimit(loadedDanmuLimit);
    setDanmuSegmentLimitInput(String(loadedDanmuLimit));
  ...
  const handleDanmuSegmentLimitChange = (v: number) => {
    const nextLimit = saveDanmuSegmentLimit(v, getSettingsUsername());
    setDanmuSegmentLimit(nextLimit);
    setDanmuSegmentLimitInput(String(nextLimit));
  };
  const commitDanmuSegmentLimitInput = () => {
    const nextLimit = sanitizeDanmuSegmentLimit(
      danmuSegmentLimitInput,
      danmuSegmentLimit,
    );
    handleDanmuSegmentLimitChange(nextLimit);
  };
  ...
  const handleResetSettings = () => {
    ...
    const defaultPreferBrowserNavigation =
      RC.PREFER_BROWSER_NAVIGATION === true;
    // 修改点：恢复默认时跟随后台站点配置的长按倍速默认值，而不是固定写死
    const defaultLockedLongPressRate = sanitizeLongPressRate(
      Number(RC.DEFAULT_LOCKED_LONG_PRESS_RATE),
    );
    ...
    setDanmuSegmentLimit(DEFAULT_DANMU_SEGMENT_LIMIT);
    setDanmuSegmentLimitInput(String(DEFAULT_DANMU_SEGMENT_LIMIT));
    ...
    localStorage.setItem(
      BROWSER_NAVIGATION_PREFERENCE_KEY,
      JSON.stringify(defaultPreferBrowserNavigation),
    );
    localStorage.setItem('playerBufferMode', 'standard');
    saveDanmuSegmentLimit(DEFAULT_DANMU_SEGMENT_LIMIT, getSettingsUsername());
    localStorage.setItem('downloadFormat', 'TS');
    localStorage.setItem(
      LOCKED_LONG_PRESS_RATE_KEY,
      String(defaultLockedLongPressRate),
    );
          ...
          <div className='space-y-6'>
            ...
            <div className='flex items-center justify-between'>
              <div>
                ...
                <p className='text-xs text-gray-500 dark:text-gray-400 mt-1'>
                  修改点：开启后，常用站内页面入口会优先使用
                  window.location.assign 进行整页跳转
                </p>
              </div>
              <Toggle
                checked={preferLocationAssignNavigation}
                onChange={handlePreferLocationAssignNavigationToggle}
              />
            </div>
                ...
                {FULLSCREEN_CLOCK_MODE_OPTIONS.map((option) => {
                  ...
                  return (
                    <button
                      ...
                      type='button'
                      onClick={() =>
                        handleFullscreenClockModeChange(option.value)
                      }
            ...
            <div className='space-y-3'>
              ...
              <div className='grid grid-cols-3 sm:grid-cols-6 gap-2'>
                {DANMU_SEGMENT_LIMIT_OPTIONS.map((option) => {
                  const isSelected = danmuSegmentLimit === option.value;
                  return (
                    <button
                      key={option.value}
                      type='button'
                      onClick={() =>
                        handleDanmuSegmentLimitChange(option.value)
                      }
                      className={`px-2 py-2 rounded-lg border text-sm font-medium transition-all duration-200 flex items-center justify-center gap-1 ${
                        isSelected
                          ? 'border-green-500 bg-green-50 text-green-700 dark:bg-green-900/20 dark:text-green-300'
                          : 'border-gray-300 text-gray-700 hover:border-gray-400 hover:bg-gray-50 dark:border-gray-600 dark:text-gray-300 dark:hover:border-gray-500 dark:hover:bg-gray-800'
                      }`}
              ...
              <div className='flex items-center gap-2'>
                <label className='shrink-0 text-xs text-gray-500 dark:text-gray-400'>
                  自定义
                </label>
                <input
                  type='number'
                  min='0'
                  step='1'
                  value={danmuSegmentLimitInput}
                  onBlur={commitDanmuSegmentLimitInput}
                  onChange={(e) => setDanmuSegmentLimitInput(e.target.value)}
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') {
                ...
                <button
                  type='button'
                  onClick={commitDanmuSegmentLimitInput}
                  className='shrink-0 px-3 py-2 rounded-lg border border-green-500 text-sm font-medium text-green-700 bg-green-50 hover:bg-green-100 transition-colors dark:bg-green-900/20 dark:text-green-300 dark:hover:bg-green-900/30'
                >
                  应用
                </button>
              </div>
