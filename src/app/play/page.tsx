...
function PlayPageClient() {
  ...
  const pendingSourceSwitchRef = useRef<PendingSourceSwitchTransaction | null>(
    null,
  );
  const sourceSwitchTransactionTokenRef = useRef(0);

  const captureCurrentPlaybackSnapshot =
    useCallback((): ActivePlaybackSnapshot | null => {
      if (
  ...
  const getActiveResourceIdentity = useCallback(() => {
    const active =
      activePlaybackSnapshotRef.current || captureCurrentPlaybackSnapshot();
    if (!active?.source || !active.id) return null;
    return {
  ...
  const rollbackPendingSourceSwitch = useCallback(
    (reason: unknown) => {
      const pending = pendingSourceSwitchRef.current;
      if (!pending) return;
  ...
  const applyCurrentFullscreenClockMode = useCallback(
  ...
  const handleToggleWatchingFollow = useCallback(async () => {
    const activeIdentity = getActiveResourceIdentity();
    if (!activeIdentity) {
    ...
    try {
      if (followingCurrentSource) {
        await deleteFollow(activeIdentity.source, activeIdentity.id);
        toast.success('已取消追更');
        return;
      }

      const activeDetail = activeIdentity.detail;
      const latestEpisodes = activeDetail?.episodes?.length ?? 0;
      if (!activeDetail || latestEpisodes <= 0) {
      ...
      await createFollow({
        ...
        title: activeDetail.title || videoTitle,
        cover: activeDetail.poster || videoCover,
        year: activeDetail.year || videoYear,
        type:
          activeDetail.type_name ||
          searchType ||
          (latestEpisodes > 1 ? 'tv' : ''),
  ...
  const updateSkipProgressMarkers = (config = skipConfigRef.current) => {
  ...
  const handleSkipConfigChange = async (newConfig: {
    ...
    try {
      ...
      if (!newConfig.enable && !newConfig.intro_time && !newConfig.outro_time) {
        await deleteSkipConfig(activeIdentity.source, activeIdentity.id);
      } else {
  ...
  const getStablePlaybackDuration = () => {
    const player = artPlayerRef.current;
    const video = player?.video as
      | (HTMLVideoElement & { hls?: any })
      | undefined;
    const hls = video?.hls;
    const hlsLevelDetails =
    ...
    const candidates = [
      ...
      Number(hlsLevelDetails?.totalduration),
    ];
  ...
  const applySkipLogic = () => {
    ...
    if (
      ...
      const outroEndTime = Math.max(0, duration - 0.1);
      updateOutroSkipHint(false);
  ...
  const saveCurrentPlayProgress = async () => {
    if (pendingSourceSwitchRef.current) {
      // Source Switch pending 期间 ArtPlayer 的 currentTime/duration 可能已
      // 来自 candidate。此时既不能生成 B PlayRecord，也不能用 B 的时间污染
      // A，所以保存直接跳过，等待 canplay/ready commit 后再按 Active 保存。
      console.log('跳过播放进度保存：候选源尚未提交');
      return;
    }

    // Source switch 未 pending 时，只有 active 快照可以进入持久化。
    ...
    try {
      ...
      const currentTotalEpisodes =
        progressSnapshot.detail?.episodes.length || 1;

      // 尝试从换源列表中获取更准确的 remarks（搜索接口比详情接口更可能有 remarks）
      const sourceFromList = availableSourcesRef.current?.find(
        (s) =>
          s.source === progressSnapshot.source && s.id === progressSnapshot.id,
      );
      const remarksToSave =
        sourceFromList?.remarks || progressSnapshot.detail?.remarks;
      ...
      savePlayRecordMutation.mutate({
        ...
        record: {
          ...
          douban_id:
            progressSnapshot.doubanId ||
            progressSnapshot.detail?.douban_id ||
            undefined,
  ...
  const findMatchedFavoriteKey = useCallback(
    ...
    [currentId, currentSource, getActiveResourceIdentity],
  );
  ...
  useEffect(() => {
    const activeIdentity = getActiveResourceIdentity();
    if (!activeIdentity?.detail) return;
    const activeDetail = activeIdentity.detail;

    const updateFavoriteData = async () => {
      try {
        const realEpisodes = activeDetail.episodes.length || 1;
        const favorites = await getAllFavorites();
        ...
        if (needsUpdate) {
          ...
          let contentType =
            favoriteToUpdate.type || inferType(activeDetail.type_name);
          ...
          saveFavoriteMutation.mutate({
            ...
            id: favId,
            favorite: {
              title: activeDetail.title || favoriteToUpdate.title,
              source_name:
                activeDetail.source_name || favoriteToUpdate.source_name || '',
              year: activeDetail.year || favoriteToUpdate.year || '',
              cover: activeDetail.poster || favoriteToUpdate.cover || '',
  ...
  const handleToggleFavorite = async () => {
    const activeIdentity = getActiveResourceIdentity();
    const activeDetail = activeIdentity?.detail;
    if (!activeDetail?.title || !activeIdentity.source || !activeIdentity.id)
      return;
    ...
    if (favorited) {
      ...
      let contentType = inferType(activeIdentity.detail.type_name);
      // 如果 type_name 无法推断类型，检查 source 是否为 shortdrama
      if (!contentType && activeIdentity.source === 'shortdrama') {
        contentType = 'shortdrama';
      }
      ...
      saveFavoriteMutation.mutate(
        {
          ...
          favorite: {
            ...
            source_name: activeDetail.source_name || '',
            year: activeDetail.year,
            cover: activeDetail.poster || '',
            total_episodes: activeDetail.episodes.length || 1,
            save_time: Date.now(),
            search_title: searchTitle,
            type: contentType,
          },
    ...
    const initPlayer = async () => {
      ...
      if (artPlayerRef.current && !loading) {
        try {
          ...
          // 🚀 换源（非切集数）成功后再加载弹幕，不再猜固定延迟
          if (!isEpisodeChange && isSourceChangingRef.current) {
            isSourceChangingRef.current = false;
            void loadDanmuAfterSourceSwitch();
          }

          // switchUrl/switchQuality resolve 只表示浏览器/ArtPlayer 接受了新
          // source，不等于视频已经可播放。真正提交 Pending Candidate 统一
          // 由 ready/canplay 事件触发，避免失败源进入 PlayRecord。
          console.log(
            '使用switch方法完成候选源加载请求，等待 ready/canplay 提交',
          );
          return;
        } catch (error) {
          ...
          isEpisodeChangingRef.current = false;
          isSourceSwitchingRef.current = false;
          // switch 失败会重建播放器，重建路径的 ready 事件里会重置换源标识
          await cleanupPlayer();
        }
      }
      if (artPlayerRef.current) {
          ...
          customType: {
            m3u8: function (video: HTMLVideoElement, url: string) {
              ...
              hls.on(Hls.Events.ERROR, function (event: any, data: any) {
                ...
                // 旧版/不兼容的代理可能只产生连续非 fatal 网络错误，这里主动降级。
                if (
                  !data.fatal &&
  ...
  const stopLockedLongPressRate = useCallback(() => {
    ...
    const player = artPlayerRef.current;
    const restoreRate = sanitizePlaybackRate(
  ...
  const isLockedLongPressIgnoredTarget = useCallback(
