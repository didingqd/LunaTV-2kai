...
import {
  ...
  DANMU_SEGMENT_LIMIT_STORAGE_PREFIX,
...
export function useDanmu(options: UseDanmuOptions): UseDanmuReturn {
  ...
  const rawDanmuListRef = useRef<any[]>([]);
  const rawDanmuRequestKeyRef = useRef<string>('');
  const rawDanmuReadyRef = useRef(false);
  // 自动重试追踪
  ...
  const getCurrentDanmuSegmentLimit = useCallback(() => {
    const username = getAuthInfoFromBrowserCookie()?.username?.trim() || null;
    return loadDanmuSegmentLimit(username);
  }, []);

  const applyCurrentDanmuSegmentLimit = useCallback(
    (danmu: any[]) => {
      const nextLimit = getCurrentDanmuSegmentLimit();
      return {
        limit: nextLimit,
        data: applyDanmuSegmentLimit(danmu, nextLimit),
      };
    },
    [getCurrentDanmuSegmentLimit],
  );
  ...
  useEffect(() => {
    ...
    const refreshVisibleDanmu = () => {
      if (
        !rawDanmuReadyRef.current ||
        rawDanmuListRef.current.length === 0 ||
        rawDanmuRequestKeyRef.current !== lastDanmuLoadKeyRef.current
      ) {
        return;
      }

      const { limit, data } = applyCurrentDanmuSegmentLimit(
        rawDanmuListRef.current,
      );
      setDanmuList(data);
      setLoadMeta((prev) => ({
        ...prev,
      ...
      console.log('📏 弹幕 segment 限制已更新:', {
        limit,
        rawCount: rawDanmuListRef.current.length,
        visibleCount: data.length,
      });
    };

    const handleLimitChange = () => refreshVisibleDanmu();
    const handleStorage = (event: StorageEvent) => {
      if (
        event.key &&
        !event.key.startsWith(DANMU_SEGMENT_LIMIT_STORAGE_PREFIX)
      ) {
        return;
      }
      refreshVisibleDanmu();
  ...
  }, [applyCurrentDanmuSegmentLimit, artPlayerRef]);
  ...
  const loadExternalDanmu = useCallback(
    async (options?: {
      ...
      rawDanmuReadyRef.current = false;
      ...
      try {
        ...
        if (!force) {
          ...
          if (cached && now - cached.timestamp < DANMU_CACHE_DURATION * 1000) {
            ...
            rawDanmuListRef.current = cached.data;
            rawDanmuRequestKeyRef.current = requestKey;
            rawDanmuReadyRef.current = true;
            const { limit, data: visibleDanmu } = applyCurrentDanmuSegmentLimit(
              cached.data,
            );
            danmuLoadingRef.current = false;
            setLoading(false);
            setDanmuList(visibleDanmu);
            setLoadMeta({
              source: 'cache',
              loadedAt: cached.timestamp,
              count: visibleDanmu.length,
            });
            console.log('📏 缓存弹幕 segment 限制:', {
              limit,
              rawCount: cached.data.length,
              visibleCount: visibleDanmu.length,
            });
            return { count: visibleDanmu.length, data: visibleDanmu };
          }
        }
        ...
        rawDanmuListRef.current = finalDanmu;
        rawDanmuRequestKeyRef.current = requestKey;
        rawDanmuReadyRef.current = true;
        const { limit, data: visibleDanmu } =
          applyCurrentDanmuSegmentLimit(finalDanmu);
        setDanmuList(visibleDanmu);
        setLoadMeta({
        ...
        console.log('📏 网络弹幕 segment 限制:', {
          limit,
          rawCount: finalDanmu.length,
          visibleCount: visibleDanmu.length,
        });
        return { count: visibleDanmu.length, data: visibleDanmu };
      } catch (error) {
